...
public class Client {
    public static void main(String... args) throws InterruptedException {
        ...
        for (int i = 0; i < 5; i++) {
            BatchRecord batchRecord = BatchRecord.newBuilder().setPartition(0).addRecords(
                Record.newBuilder().setKey("rk").setValue(ByteString.copyFrom(("hello world " + i).getBytes())).build())
                .build();
        ...
        for (int i = 0; i< 5;i++) {
            ...
            stub.get(GetRequest.newBuilder().setTopic("TP_FOO").setPartition(0).setOffset(i).build(), new StreamObserver<GetResponse>() {
                @Override
